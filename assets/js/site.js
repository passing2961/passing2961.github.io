...
(function () {
  ...
  const NAV = [
    ...
    { id: "research", label: "Research Statement", href: "research.html" },
  ...
  const PUB_LINK_ICONS = {
    project: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="14" rx="2"/><path d="M3 9h18"/><path d="M9 21h6"/><path d="M12 18v3"/></svg>`,
    ...
    dataset: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><ellipse cx="12" cy="5" rx="8" ry="3"/><path d="M4 5v6c0 1.7 3.6 3 8 3s8-1.3 8-3V5"/><path d="M4 11v6c0 1.7 3.6 3 8 3s8-1.3 8-3v-6"/></svg>`,
    models: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m12 2 9 5-9 5-9-5 9-5Z"/><path d="m3 12 9 5 9-5"/><path d="m3 17 9 5 9-5"/></svg>`,
  ...
  const PUB_LINK_META = {
    project: { cls: "pub-link-project", icon: "project", label: "Project", title: "Project page" },
    paper:   { cls: "pub-link-paper", icon: "paper", label: "Paper",   title: "Read the paper" },
    code:    { cls: "pub-link-code",  icon: "code",  label: "Code",    title: "View the source code" },
    dataset: { cls: "pub-link-data",   icon: "dataset", label: "Dataset", title: "View the dataset" },
    models:  { cls: "pub-link-models", icon: "models",  label: "Models",  title: "View the models" },
  };

  function pubLinksHtml(links) {
    if (!links) return "";
    const order = ["project", "paper", "code", "dataset", "models"];
    const btns = order
      .filter((k) => links[k])
      .map((k) => {
        const base = PUB_LINK_META[k];
        const isHuggingFace = (k === "dataset" || k === "models")
          && /^https:\/\/(?:www\.)?huggingface\.co\//i.test(links[k]);
        const m = isHuggingFace ? { ...base, cls: "pub-link-hf", icon: "hf" } : base;
  ...
  function pubItemHtml(p, idx, showNum = true) {
    const note = p.note ? `<span class="pub-note-inline">${p.note}</span>` : "";
    const num = showNum ? `<div class="pub-num">${idx}</div>` : "";
    return `<li class="pub-item${showNum ? "" : " no-num"}" id="pub-${p.id}">
      ${num}
      <div>
  ...
  function renderSelectedPubs() {
    const items = S.publications.selected
      .map((id) => findPub(id))
      .filter(Boolean)
      .map((p) => pubItemHtml(p, 0, false))
      .join("");
    return `<ul class="pub-list no-num">${items}</ul>`;
  }
  ...
  function renderResearch() {
    const mount = document.getElementById("research");
    if (!mount) return;
    mount.innerHTML = `<h1 class="page-title">Research Statement</h1>`;
  }
